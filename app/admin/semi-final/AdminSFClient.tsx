...
import { lockSemiFinalPredictions, calculateSemiFinalScores } from "@/lib/admin-actions"
import { Lock, Calculator, Loader2 } from "lucide-react"

export default function AdminSFClient() {
    const [isLocking, setIsLocking] = useState(false)
    const [isScoring, setIsScoring] = useState(false)
    const [message, setMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null)

    const handleLock = async () => {
        if (!confirm("Are you sure you want to lock all Semi-Final predictions globally? Users will not be able to change their predictions after this.")) return

        setIsLocking(true)
        setMessage(null)
        const res = await lockSemiFinalPredictions()
        setIsLocking(false)
    ...
    const handleCalculate = async () => {
        if (!confirm("Are you sure you want to trigger Semi-Final scoring? This will process all users and calculate points based on finished Semi-Final matches.")) return

        setIsScoring(true)
        setMessage(null)
        const res = await calculateSemiFinalScores()
        setIsScoring(false)
                ...
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm mt-1 mb-4">
                            Manually lock all Semi-Final predictions. This is an override if the global deadline fails or if you want to lock early.
                        </p>
                    </div>
                    <button
                        onClick={handleLock}
            ...
            <div className="bg-card border border-border p-6 rounded-2xl">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm mt-1 mb-4">
                            Trigger the scoring engine for the Semi-Finals. This will fetch all finished Semi-Final matches and award points to users based on their confidence multipliers (1x, 2x, 3x).
                        </p>
                    </div>
                    <button
