...
interface GrandFinalHomeProps {
    initialMatches: Match[]
    gfFinalists: { id: string; name: string; seed: number }[]
    gfDeadline: string | null
    sfRecap: { id: string; winnerName: string; winnerScore: number | null }[]
    grandFinalTop: { username: string; points: number }[]
...
export function GrandFinalHome({ initialMatches, gfFinalists, gfDeadline, sfRecap, grandFinalTop, quarterFinalTop }: GrandFinalHomeProps) {
    const [searchQuery, setSearchQuery] = useState("")
    const countdown = useCountdown(gfDeadline)
    ...
    const standingsSource = grandFinalTop.length > 0 ? grandFinalTop : quarterFinalTop
    const standingsLabel = grandFinalTop.length > 0 ? "Grand Final · Live Standings" : "Playoff · Quarter-Final Total"
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col gap-10">
            ...
            <section className="relative rounded-[2rem] p-6 md:p-10 lg:p-12 overflow-hidden bg-card border border-border/60 shadow-xl">
                <div className="absolute -top-32 -right-32 w-[28rem] h-[28rem] rounded-full bg-amber-500/10 blur-3xl pointer-events-none" />
                <div className="absolute -bottom-40 -left-24 w-96 h-96 rounded-full bg-primary/10 blur-3xl pointer-events-none" />

                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-8 lg:gap-12 items-center">
                    {/* Left: copy */}
                    <div className="flex flex-col gap-5">
                        <div className="flex flex-wrap items-center gap-2">
                            <span className="inline-flex items-center gap-1.5 text-[10px] font-black tracking-widest text-amber-500 uppercase bg-amber-500/10 border border-amber-500/25 px-3 py-1.5 rounded-full">
                                <StarIcon className="h-3.5 w-3.5 text-amber-500" /> Grand Final · Sep 10
                            </span>
                        </div>

                        <h1 className="text-4xl md:text-5xl font-black font-russo tracking-tight leading-[1.05]">
                            The Grand<br />
                            <span className="text-amber-500">Finale</span> Awaits
                        </h1>

                        <p className="text-muted-foreground text-sm md:text-md leading-relaxed max-w-lg">
                            Three champions, one winner. Pick the champion, runner-up and final margin to bank
                            up to 290 points in the Ultimate Predictor.
                        </p>

                        <div className="flex flex-col sm:flex-row gap-3 pt-1">
                            <Link
                                href="/fantasy/grand-final"
                                className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-sm uppercase tracking-wider transition-standard hover:-translate-y-0.5 active:translate-y-0 shadow-lg shadow-amber-500/20"
                            >
                                Make Your Predictions <ChevronRightIcon className="h-4 w-4" />
                            </Link>
                            <Link
                                href="/leaderboard"
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="rounded-2xl border border-border/60 bg-background/50 p-4 flex items-center justify-between gap-4">
                            <div className="flex items-center gap-2 text-muted-foreground">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest">Final Kickoff</span>
                            </div>
                        ...
                        <Link
                            href="/fantasy/grand-final"
                            className="group rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 flex flex-col gap-3 hover:border-amber-500/50 hover:bg-amber-500/10 transition-standard"
                        >
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] font-black uppercase tracking-widest text-amber-500">The Finalists</span>
                                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{formatContestDate(gfDeadline)}</span>
                            </div>
                            <ol className="flex flex-col gap-1.5">
                                {gfFinalists.map((f, i) => (
                                    <li key={f.id} className="flex items-center gap-2.5 text-sm font-extrabold text-foreground">
                                        <span className="shrink-0 w-6 h-6 rounded-lg bg-amber-500/15 border border-amber-500/30 flex items-center justify-center text-[10px] font-black text-amber-500">
                                            {i + 1}
                                        </span>
                                        {f.name}
                                    </li>
                            ...
                            <span className="inline-flex items-center gap-1.5 text-[11px] font-black text-amber-500 uppercase tracking-wider">
                                Predict the Final <ChevronRightIcon className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
                            </span>
                        </Link>
                    </div>
                </div>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-border border-y border-border">
                <Link href="/fantasy/grand-final" className="group flex items-start gap-4 py-6 sm:px-6 first:sm:pl-0 hover:bg-primary/5 transition-standard">
                    <BoltIcon className="h-6 w-6 text-amber-500 shrink-0 mt-0.5 transition-transform group-hover:-translate-y-0.5" />
                    <div>
                        <h3 className="font-extrabold text-base text-foreground">Ultimate Predictor</h3>
                        <p className="text-muted-foreground text-xs mt-1 leading-relaxed">Champion, runner-up, margin and boost. Up to 290 points.</p>
                    </div>
                </Link>
                ...
                <Link href="/chat" className="group flex items-start gap-4 py-6 sm:px-6 hover:bg-primary/5 transition-standard">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground text-xs mt-1 leading-relaxed">Claim your school badge. Talk the final live with fellow fans.</p>
                    </div>
                </Link>
                ...
                <Link href="/leaderboard" className="group flex items-start gap-4 py-6 sm:px-6 last:sm:pr-0 hover:bg-primary/5 transition-standard">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground text-xs mt-1 leading-relaxed">QF + SF points are banked. Watch the title race heat up.</p>
                    </div>
                </Link>
            </section>
            ...
            {sfRecap.length > 0 && (
                <section className="flex flex-col gap-5">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-2xl font-black font-russo uppercase tracking-tight text-foreground">Road to the Final</h2>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {sfRecap.map((r, idx) => (
                            <div key={r.id} className="rounded-3xl border border-border/60 bg-card p-5 flex items-center gap-4">
                                <span className="shrink-0 w-11 h-11 rounded-2xl bg-amber-500/10 border border-amber-500/25 flex items-center justify-center text-[10px] font-black text-amber-500 uppercase">
                                    SF {idx + 1}
                                </span>
                                ...
                                <span className="text-xl font-black text-amber-500 tabular-nums">{r.winnerScore}</span>
                            </div>
                        ))}
                    </div>
                </section>
                        ...
                        {standingsSource.map((row, idx) => {
                            ...
                            return (
                                <div key={row.username} className="flex items-center gap-4 py-3">
                                    ...
                                    <span className="ml-auto font-black text-amber-500 tabular-nums">{row.points} pts</span>
                                </div>
                            )
                        })}
                    </div>
