...
export function ApprovalQueue({ initialPending, matches }: { initialPending: PendingResult[], matches: any[] }) {
    ...
    if (pending.length === 0) {
        return <div className="p-8 text-center bg-card rounded-xl">No pending results in the queue.</div>
    }
    ...
    return (
        <div className="space-y-6">
            {pending.map(item => {
                ...
                return (
                    <div key={item.id} className="p-6 bg-card rounded-xl border border-border space-y-4">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                {parsed.round && <p className="text-muted-foreground text-sm">Round: {parsed.round}</p>}
                            </div>
                        ...
                        <div className="p-3 bg-muted rounded-lg text-sm text-foreground/80 border border-border">
                        ...
                        <div className="flex items-center gap-4 pt-2">
                            <select
                                className="bg-muted text-foreground rounded-md px-3 py-2 text-sm flex-1 border border-input"
